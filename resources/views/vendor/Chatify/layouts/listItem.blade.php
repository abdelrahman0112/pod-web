        ...
        <div class="flex items-center space-x-3">
            ...
            <div class="flex-shrink-0">
                <div class="saved-messages w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center">
                    <i class="ri-bookmark-line text-indigo-600 text-lg"></i>
                </div>
            </div>
...
<div class="messenger-list-item px-3 py-3 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors" data-contact="{{ $user->id }}">
    <div class="flex items-center space-x-3">
        ...
        <div class="flex-shrink-0 relative" style="padding: 4px; margin-right: 4px;">
            <x-chatify-avatar 
                ...
                <p class="text-xs text-slate-500 truncate flex-1">
                    ...
                    @else
                        <span class="inline-flex items-center">
                            <i class="ri-file-line mr-1"></i> Attachment
                        </span>
                    @endif
                </p>
                {{-- Unread counter --}}
...
{{-- -------------------- Shared photos Item -------------------- --}}
@if($get == 'sharedPhoto')
<div class="shared-photo rounded-lg overflow-hidden cursor-pointer hover:opacity-90 transition-opacity w-full aspect-square">
    <img src="{{ $image }}" 
         alt="Shared photo" 
         class="w-full h-full object-cover rounded-lg"
         onerror="this.style.display='none'; this.parentElement.innerHTML='<div class=\\'w-full h-full bg-slate-100 rounded-lg flex items-center justify-center\\'><i class=\\'ri-image-line text-slate-400\\'></i></div>';">
</div>
@endif
